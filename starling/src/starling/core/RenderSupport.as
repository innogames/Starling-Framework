...
package starling.core
{
    ...
    public class RenderSupport
    {
		...
		private static var sCurrentProgramName:String ="none";
		private static var sCurrentProgram:Program3D;
		private static var sCurrentBlendFactors:Array;
        ...
        final public function finishQuadBatch():void
        {
        ...
        public static function setBlendFactors(premultipliedAlpha:Boolean, blendMode:String="normal"):void
        {
			...
			if (sCurrentBlendFactors != blendFactors)
			{
				sCurrentBlendFactors = blendFactors;
		...
		public static function setProgram(program3D:Program3D):void
		{
			if (sCurrentProgram != program3D)
			{
				var context:Context3D = Starling.context;
				if (context == null) throw new MissingContextError();
				sCurrentProgramName = "none";
				sCurrentProgram = program3D;
				context.setProgram(program3D);
				
			}
		}
		public static function setProgramByName(programName:String):void
		{
			if (sCurrentProgramName != programName)
			{
				var program:Program3D = Starling.current.getProgram(programName);
				if (program)
				{
					var context:Context3D = Starling.context;
					if (context == null) throw new MissingContextError();
					sCurrentProgramName = programName;
