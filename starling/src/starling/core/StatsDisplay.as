...
package starling.core
{
    ...
    internal class StatsDisplay extends Sprite
    {
		...
		private static const sAddionalData:Dictionary = new Dictionary();

        /** Creates a new Statistics Box. */
        public function StatsDisplay()
        {
            mBackground = new Quad(90, 35, 0x0);
            mTextField = new TextField(78, 35, "", BitmapFont.MINI, BitmapFont.NATIVE_SIZE, 0xffffff);
            mTextField.x = 2;
        ...
        public function update():void
        {
			...
			var str:String = "FPS: " + mFps.toFixed(mFps < 100 ? 1 : 0) +
							"\nMEM: " + mMemory.toFixed(mMemory < 100 ? 1 : 0) +
							"\nDRW: " + (mTotalTime > 0 ? mDrawCount-2 : mDrawCount);

			for(var key:String in sAddionalData)
			{
				str += "\n" + key +  ": " + (sAddionalData[key] as Number).toFixed();
				++numLines;
			}

			mBackground.height = 9 * numLines;
			mTextField.height = 9 * numLines;
