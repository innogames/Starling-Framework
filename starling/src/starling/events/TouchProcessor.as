...
package starling.events
{
    ...
    public class TouchProcessor
    {
        ...
        /** A vector of TouchArgs with the arguments that were passed to the "enqueue"
         *  method (the oldest being at the end of the vector). */
        protected var mQueue:Vector.<TouchArgs>;
        ...
        public function advanceTime(passedTime:Number):void
        {
            ...
            while (mQueue.length > 0)
            {
                ...
                while (mQueue.length > 0 &&
                      !containsTouchWithID(sUpdatedTouches, mQueue[mQueue.length-1].touchID))
                {
                    var touchArgs:TouchArgs = mQueue.pop();
                    touch = createOrUpdateTouch(
                                touchArgs.touchID, touchArgs.phase, touchArgs.globalX, touchArgs.globalY,
								touchArgs.pressure, touchArgs.width, touchArgs.height);
					TouchArgs.toPool(touchArgs);
                                          ...
                                          shiftDown:Boolean, ctrlDown:Boolean):void
        {
            ...
            var touchEvent:TouchEvent = TouchEvent.fromPool(TouchEvent.TOUCH, touches, shiftDown, ctrlDown);
            var touch:Touch;
        ...
        {
            mQueue.unshift(TouchArgs.fromPool(touchID, phase, globalX, globalY, pressure, width, height));
            ...
            if (mCtrlDown && simulateMultitouch && touchID == 0)
            {
                ...
                mQueue.unshift(TouchArgs.fromPool(1, phase, mTouchMarker.mockX, mTouchMarker.mockY));
            }
        }
        ...
        private function onKey(event:KeyboardEvent):void
        {
            if (event.keyCode == 17 || event.keyCode == 15) // ctrl or cmd key
            {
                ...
                if (simulateMultitouch && wasCtrlDown != mCtrlDown)
                {
                    ...
                    {
                        // end active touch ...
                        mQueue.unshift(TouchArgs.fromPool(1, TouchPhase.ENDED, mockedTouch.globalX, mockedTouch.globalY));
                    }
                    else if (mCtrlDown && mouseTouch)
                    {
                        // ... or start new one
                        if (mouseTouch.phase == TouchPhase.HOVER || mouseTouch.phase == TouchPhase.ENDED)
                            mQueue.unshift(TouchArgs.fromPool(1, TouchPhase.HOVER, mTouchMarker.mockX, mTouchMarker.mockY));
                        else
                            mQueue.unshift(TouchArgs.fromPool(1, TouchPhase.BEGAN, mTouchMarker.mockX, mTouchMarker.mockY));
                    }
                }
            }
...
/**
 * helper class to avoid garbage in touch processing
 */
class TouchArgs
{
	private static var sTouchArgsPool:Vector.<TouchArgs> = new Vector.<TouchArgs>();
	...
	public static function fromPool(touchID:int, phase:String, globalX:Number, globalY:Number, pressure:Number = 1.0,
									width:Number = 1.0, height:Number = 1.0):TouchArgs
	{
		if (sTouchArgsPool.length)
			return sTouchArgsPool.pop().reset(touchID, phase, globalX, globalY, pressure, width, height);
		else
			return new TouchArgs().reset(touchID, phase, globalX, globalY, pressure, width, height);
	}
	
	internal function reset(touchID:int, phase:String, globalX:Number, globalY:Number, pressure:Number, width:Number, height:Number):TouchArgs
	{
		_height = height;
		_width = width;
		_pressure = pressure;
	...
	public static function toPool(touchArgs:TouchArgs):void
	{
		sTouchArgsPool[sTouchArgsPool.length] = touchArgs;
	}
