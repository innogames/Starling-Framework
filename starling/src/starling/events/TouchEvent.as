...
package starling.events
{
    ...
    public class TouchEvent extends Event
    {
        ...
        protected var mShiftKey:Boolean;
        protected var mCtrlKey:Boolean;
        protected var mTimestamp:Number;
        protected var mVisitedObjects:Vector.<EventDispatcher>;
                                   ...
                                   ctrlKey:Boolean=false, bubbles:Boolean=true)
        {
            ...
            reset(type, touches, shiftKey, ctrlKey, bubbles);
        }
		...
		/** @private */
        internal static function fromPool(type:String, touches:Vector.<Touch>, shiftKey:Boolean = false,
											ctrlKey:Boolean=false, bubbles:Boolean=true):TouchEvent
        {
            if (sEventPool.length) return sEventPool.pop().reset(type, touches, shiftKey, ctrlKey, bubbles);
            else return new TouchEvent(type, touches, shiftKey, ctrlKey, bubbles);
        }

        /** @private */
        internal static function toPool(event:TouchEvent):void
        {
			event.mData = event.mTarget = event.mCurrentTarget = null;
			event.mVisitedObjects.length = 0;
            sEventPool[sEventPool.length] = event; // avoiding 'push'
        }

        /** @private */
        internal function reset(type:String, touches:Vector.<Touch>, shiftKey:Boolean = false,
											ctrlKey:Boolean=false, bubbles:Boolean=true):TouchEvent
        {
